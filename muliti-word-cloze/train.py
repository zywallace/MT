...
# python train_bi.py --data_file data/hw4_data.bin --optimizer Adam -lr 1e-2 --batch_size 48  --model_file model/test.model --gpuid [0]
...
def main(options):
    ...
    batched_train_in, batched_train_in_mask, _ = utils.tensor.advanced_batchize(train_in, options.batch_size,
                                                                                vocab.stoi["<pad>"])
    batched_train_out, batched_train_out_mask, _ = utils.tensor.advanced_batchize(train_out, options.batch_size,
                                                                                  vocab.stoi["<pad>"])
    batched_dev_in, batched_dev_in_mask, _ = utils.tensor.advanced_batchize(dev_in, options.batch_size,
                                                                            vocab.stoi["<pad>"])
    batched_dev_out, batched_dev_out_mask, _ = utils.tensor.advanced_batchize(dev_out, options.batch_size,
                                                                              vocab.stoi["<pad>"])
    ...
    for epoch_i in range(options.epochs):
        ...
        for i, batch_i in enumerate(utils.rand.srange(len(batched_train_in))):
            ...
            # change model
            sys_out_batch = rnnlm(train_in_batch)
        ...
        if (last_dev_avg_loss - dev_avg_loss).data[0] < options.estop:
            logging.info(
                "Early stopping triggered with threshold {0} (previous dev loss: {1}, current: {2})".format(epoch_i,
                                                                                                            last_dev_avg_loss.data[
                                                                                                                0],
                                                                                                            dev_avg_loss.data[
                                                                                                                0]))
            break
        torch.save(rnnlm,
                   open(options.model_file + ".nll_{0:.2f}.epoch_{1}".format(dev_avg_loss.data[0], epoch_i), 'wb'),
                   pickle_module=dill)
